...
const calculateConsineSimilarity = (userFrame: Frame, serverFrame: Frame): number => {
    ...
    const temp = calculateDot(sArray, uArray, 1) / (calculateNorm(sArray) * calculateNorm(uArray));
    const score = ((2 - Math.sqrt(2 * (1 - temp))) / 2) * 100
    
    return score;
}

const calculateDot = (array1: number[], array2: number[], ndarray: number): number => {
    if (ndarray == 1) {
        let answer = 0;
...
const compareFrames = (
    ...
    for (let i = 0; i < userFrames.length; i++) {
        let lowestScore = 100;
        let highestScore = 0;
        let lowestScoreFrame: Frame = {
            'frameId': 0, 'keypoints': [], 'box': []
        };
        const userFrame = userFrames[i];
        for (let j = 0; j < serverFrames.length; j++) {
            const serverFrame = serverFrames[j];
            const tempScore = calculateConsineSimilarity(userFrame, serverFrame);
